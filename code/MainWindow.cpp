...
void MainWindow::writeExcel() {
	...
	vector<QString> faceInfo = sqlSplite->getFaceInfoNos();
	// 路径设置
	...
	if (!filePath.exists()) {
		...
		for (int i = 0; i <= faceInfo.size(); i++) {
			...
			if (i == 0) {
				...
				aRowData.append(QVariant(SQ("是否签到")));
			}
			else {
				QStringList faceData = faceInfo[i - 1].split("|");
				QStringList data;
				bool flag = false;
				for (int row = 0; row <  datas.size(); row++) {
					data = datas[row].split("|");
					if (data[2] == faceData[2]) {
						flag = true;
						break;
					}
				}
				for (int column = 0; column < faceData.size(); column++)
				{
					aRowData.append(QVariant(faceData[column]));
				}
				aRowData.append(QVariant(data[3]));
				if (flag) {
					aRowData.append(data[4]);
					aRowData.append(data[5]);
					aRowData.append(QVariant(SQ("已签到")));
				}
				else {
					aRowData.append(data[4]);
					aRowData.append("");
					aRowData.append(QVariant(SQ("未签到")));
				}
		...
		string num = QS(QString::number(faceInfo.size()+1));
		QString col = (("A1:G"+num).c_str());
